...
function toDoc(raw) {
  return {
    ...
    homonym_number: Number.isInteger(raw.homonym_number) ? raw.homonym_number : null,
...
async function seedIfEmpty() {
  ...
  if (count > 0) {
    console.log(`[seed] Koleksi words sudah berisi ${count} dokumen — lewati seeding`);
    return;
  }
  ...
  if (inserted !== docs.length) {
    console.warn(`[seed] PERINGATAN: hanya ${inserted}/${docs.length} dokumen yang masuk`);
  }
