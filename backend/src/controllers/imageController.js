...
exports.uploadImage = async (req, res, next) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json({ message: 'File gambar (field "image") wajib diunggah' });
    }
    ...
    res
      .status(201)
      .json({ message: "Gambar berhasil diunggah ke MinIO", data: word });
  } catch (err) {
    next(err);
  }
};
...
exports.deleteImage = async (req, res, next) => {
  try {
    ...
    if (!word.image || !word.image.objectName) {
      return res
        .status(404)
        .json({ message: "Kata ini tidak memiliki gambar" });
    }
...
exports.getImage = async (req, res, next) => {
  try {
    ...
    if (err.code === "NoSuchKey" || err.code === "NotFound") {
      return res
        .status(404)
        .json({ message: "Gambar tidak ditemukan di MinIO" });
    }
    next(err);
  }
};
